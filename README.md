...
- `reference_energies.jsonc` is now functional-keyed (`gases` + `elements`) and selected with `--functional`; H2/H2O energies moved out of the code into that file
- Per-element reference models (metal/oxide/…) selected with `--ref-model`, backed by the `reference_energies/` structure database and `build_reference_energies.py`
